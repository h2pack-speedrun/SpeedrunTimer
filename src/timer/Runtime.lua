...
function internal.RegisterHooks()
    modutil.mod.Path.Wrap("StartNewRun", function(baseFunc, prevRun, args)
        if not lib.coordinator.isEnabled(store, public.definition.modpack) then return baseFunc(prevRun, args) end
        if activeTimer then
            StopAndCleanup()
        end
    ...
    modutil.mod.Path.Wrap("RoomEntranceMaterialize", function(baseFunc, ...)
        if not lib.coordinator.isEnabled(store, public.definition.modpack) then return baseFunc(...) end
        local val = baseFunc(...)
        ...
        if activeTimer and activeTimer.Running and not updateThreadActive then
            ...
            thread(function()
                while activeTimer and activeTimer.Running do
                    if not lib.coordinator.isEnabled(store, public.definition.modpack) then
                        StopAndCleanup()
                        return
                    end
                    activeTimer:update()
    ...
    modutil.mod.Path.Wrap("AddTimerBlock", function(baseFunc, currRun, timerBlockName)
        ...
        if lib.coordinator.isEnabled(store, public.definition.modpack) and timerBlockName == "MapLoad"
